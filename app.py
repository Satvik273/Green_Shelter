from flask import Flask, render_template, request, flash, redirect, url_for, jsonify, make_response, Response, send_from_directory
...
def init_db_command():
    ...
    # Ensure all tables are created. This is safe to run even if tables exist.
    # It's necessary here because the seeding check below queries the database.
    db.create_all()
...
def sitemap():
    """Generates and serves the sitemap.xml file dynamically."""
    pages = []
    # Use _external=True with url_for to generate full URLs
    ...
    for product in Product.query.all():
        pages.append({
            "loc": url_for('product_detail', slug=product.slug, _external=True),
